...
class MyResearchController extends AbstractBase
{
    ...
    public function profileAction()
    {
        ...
        if (stristr($patron['scope'], 'read_patron')) {
           ...
           return $this->createViewModel(array('error' => 'SCOPE_MISSING_READ_PATRON'));
        }
    }
    ...
    public function mylistAction()
    {
        ...
        try {
            $results = $this->pluginManager->get('Favorites');
...
#                    ->lightboxAwareRedirect($details['route'], $details['params']);
    ...
    public function deletelistAction()
    {
        ...
        return $this->createViewModel(array('list_id' => $listID));
    }
    ...
    public function holdsAction()
    {
        ...
        if (stristr($patron['scope'], 'read_items')) {
           // Connect to the ILS:
           $catalog = $this->getILS();
           ...
           $result = $catalog->getMyHolds($patron, array(2,4));
           $recordList = array();
           ...
           foreach ($result as $current) {
               ...
               if ($cancelStatus && $cancelStatus['function'] != "getCancelHoldLink"
                   ...
                   if (stristr($patron['scope'], 'write_items')) {
    ...
    public function finesAction()
    {
        ...
        if (stristr($patron['scope'], 'read_fees')) {
           // Connect to the ILS:
           $catalog = $this->getILS();
           ...
           return $this->createViewModel(array('error' => 'SCOPE_MISSING_READ_FEES'));
        }
    }
    ...
    public function patronAction() {
       ...
       if (!$this->isLoggedIn()) {
          if (isset($_POST['username']) && isset($_POST['password'])) {
             $json = $paia->login($_POST['username'], $_POST['password']);
             if ($this->debug) {
                $result['json'] = $json;
             }
             ...
             if (!isset($json_array['error'])) {
                ...
                $result['access_token'] = $json_object->access_token;
                setcookie('patron', $json_object->patron, time()+3600, '/');
                setcookie('paia_access_token', $json_object->access_token, time()+3600, '/');
             } else {
                $result['error'] = $json_array['error_description'];
             }
          }
       } else {
          $result['showLogin'] = false;
          if ($this->debug) {
             $result['get_array'] = $_GET;
             $result['post_array'] = $_POST;
          }
          
          if(isset($_GET['logout'])) {
             $json = $paia->logout($_COOKIE['patron']);
             if ($this->debug) {
                $result['json'] = $json;
             }
             $json_array = json_decode($json, true);
    ...
    function isLoggedIn() {
       if (isset($_COOKIE['paia_access_token'])) {
          return true;
       } else {
          return false;
       }
    ...
    protected function catalogLogin()
    {
        ...
        if (($username = $this->params()->fromPost('cat_username', false))
            ...
            if (method_exists($account, 'storedCatalogLogin')) {
     ...
     * @return view
     */
    public function catalogLogoutAction()
    {
        // First make sure user is logged in to VuFind:
        $account = $this->getAuthManager();
        if ($account->isLoggedIn() == false) {
     ...
     * @return view
     */
    public function catalogOrderAction()
    {
        // First make sure user is logged in to VuFind:
        $account = $this->getAuthManager();
        if ($account->isLoggedIn() == false) {
        ...
        $paia = $this->getILS()->getDriver();
        $paia->order($_GET['recordId'], $patron);
    }
    
    public function placeHoldAction () {
        // First make sure user is logged in to VuFind:
        $account = $this->getAuthManager();
        if ($account->isLoggedIn() == false) {
            return $this->forceLogin();
        ...
        $documentId = urldecode($_GET['documentId']);
        $itemId = urldecode($_GET['itemId']);
        $type = $_GET['type'];
        
        $paia = $this->getILS()->getDriver();
        $orderJson = $paia->order($documentId, $itemId, $patron);
        
        $orderJsonArray = json_decode($orderJson->getBody());
        
        $view = $this->createViewModel();

		$generic_error_message = "Die Bestellung, Vormerkung oder Verlängerung konnte nicht durchgeführt werden. Bitte notieren Sie sich den Titel, an dem Sie interessiert sind, und fragen Sie an einer unserer Informations- und Servicestellen nach.";
		$catalog = $this->getILS();
		$profile = $catalog->getMyProfile($patron);
		$status = $profile['status'];
		
        if ($orderJsonArray->doc[0]->error) {
			if($orderJsonArray->doc[0]->error != "") {
				if($orderJsonArray->doc[0]->error == 'insufficient_scope' && !empty($status)) {
					$view->error = $this->translate('Fehler: '.$orderJsonArray->doc[0]->error."-".$status);	
				} else {
					$view->error = $this->translate('Fehler: '.$orderJsonArray->doc[0]->error);			
				}
			} else {
				$view->error = 'Fehler: '.$generic_error_message;
			}
			
        } elseif ($orderJsonArray->error) {	
			if($orderJsonArray->error != "") {
								if($orderJsonArray->error == 'insufficient_scope' && !empty($status)) {
					$view->error = $this->translate('Fehler: '.$orderJsonArray->error."-".$status);	
				} else {
					$view->error = $this->translate('Fehler: '.$orderJsonArray->error);			
				}
			} else {
				$view->error = $this->translate('Fehler: '.$generic_error_message);
			}
			
		} else {
            if ($type == 'recall') {
                return $this->redirect()->toUrl('Recalls?placeRecall=success'.'&'.$orderJsonArray->doc[0]->code);
            } else {
                return $this->redirect()->toUrl('Holds?placeHold=success'.'&'.$orderJsonArray->doc[0]->code);
            }
        }
