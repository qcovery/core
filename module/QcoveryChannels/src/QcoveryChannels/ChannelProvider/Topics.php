 ...
 * @link     https://vufind.org/wiki/development Wiki
 */
class Topics extends \VuFind\ChannelProvider\AbstractChannelProvider
   implements TranslatorAwareInterface
{
    ...
    /**
     * Facet values to use (field name => value).
     *
     * @var array
     */
    protected $values;
    ...
    public function configureSearchParams(Params $params)
    {
        ...
        foreach ($this->values as $field => $value) {
            foreach (explode(',', $value) as $valueEntry) {
                $params->addFilter($field.':"'.$valueEntry.'"');
            }
        }
    }
     ...
     * @return array
     */
    public function getFromSearch2(Results $results, $channelToken = null)
    {
        if (null !== $channelToken) {
            return [$this->buildChannelFromToken($results, $channelToken)];
        }
